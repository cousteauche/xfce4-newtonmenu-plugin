...
static void on_hide_app_name_checkbutton_toggled (GtkToggleButton *togglebutton, gpointer user_data); // NEW
...
// NEW: Callback for "Hide Application Name" checkbox
static void
on_hide_app_name_checkbutton_toggled (GtkToggleButton *togglebutton, gpointer user_data)
{
    GtkBuilder *builder = GTK_BUILDER(user_data);
...
dialog_save_settings_and_update (GtkDialog *dialog, newtonmenuPlugin *newtonmenu, GtkBuilder *builder)
{
    ...
    // NEW: Save "Hide Application Name" property
    ...
    // NEW: Save "Global Menu Title" property
    ...
    // NEW: Trigger update for combined menu when global menu settings change
                  ...
                  newtonmenuPlugin    *newtonmenu)
{
  ...
  if (GTK_IS_TOGGLE_BUTTON(widget)) {
      ...
      on_display_icon_checkbutton_toggled(GTK_TOGGLE_BUTTON(widget), builder); // Initial state
  ...
  // NEW: Global menu appearance settings
  widget = GTK_WIDGET(gtk_builder_get_object (builder, "hide_app_name_checkbutton"));
  if (GTK_IS_TOGGLE_BUTTON(widget)) {
      gtk_toggle_button_set_active (GTK_TOGGLE_BUTTON (widget), newtonmenu->hide_application_name_prop);
      on_hide_app_name_checkbutton_toggled(GTK_TOGGLE_BUTTON(widget), builder); // Initial state
  ...
  // Confirmation settings (existing code, ensure it's still correct)
  widget = GTK_WIDGET(gtk_builder_get_object (builder, "confirm_logout_checkbutton"));
  if (GTK_IS_TOGGLE_BUTTON(widget))
